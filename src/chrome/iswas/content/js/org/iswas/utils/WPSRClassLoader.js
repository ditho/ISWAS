...
(function () {
    ...
    function trigger(rdf, ns) {
        ...
        return cache.getInstance(ns).process(xml, uri, rdf);
        //return cache.getInstance(ns).process(content.document, uri, rdf);
    }
    /**
    ...
    function load(ns) {
        if (namespace) {
            dump("org.iswas.utils.WPSRClassLoader[namespace1=" + namespace + "]\n");
            ns = namespace;
        }
        //Get extension folder installation path...
        if (!extPath) {
            dump("org.iswas.utils.WPSRClassLoader[namespace2=" + ns + "]\n");
            namespace = ns;
            window.setTimeout(load, 500);
            return false;
        }
        dump("org.iswas.utils.WPSRClassLoader[extensioPath=" + extPath.path + "]\n");
        if (!ns) {
            throw new Error("org.iswas.utils.WPSRClassLoader[namespace not defined]");
        }
        dump("org.iswas.utils.WPSRClassLoader[namespace3=" + ns + "]\n");
        ...
        var classLoader = "file:///C:/Programme/cygwin/home/tb006/projects/ISWAS/src/java/javaFirefoxExtensionUtils.jar";
        ...
        // the line below have to be uncomment for the prod version of the firefox extension
        var jarPath = "file:///C:/Programme/cygwin/home/tb006/projects/ISWAS/src/java/wpsr.jar";
        ...
        //var cl = new java.net.URLClassLoader([new java.net.URL(jarPath)]);
        ...
        cache.setInstance(ns, myClass.newInstance());
        return true;
    }
    // DEFINE THE @private SYMBOLS FOR THE CLOSURE
    // note it is quite good to set an underscore before each symbol
    var cache = InstanceCache.create("org.iswas.utils.WPSRClassLoader");
    var namespace = "";
    var extPath = "";
    AddonManager.getAddonByID("iswas@uni-rostock.de", function (addon) {
        extPath = addon.getResourceURI("").QueryInterface(Components.interfaces.nsIFileURL).file;
    });
    /**
     ...
     * @return
     */
    function addPolicy(loader, urls) {
        try {

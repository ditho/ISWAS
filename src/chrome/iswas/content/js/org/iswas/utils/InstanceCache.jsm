...
(function(){
    ...
    function create(key) {
        ...
        if(_crossCache.hasInstance(key)) {
            dump("org.iswas.utils.InstanceCache.create[get Instance for key=" + key + "]\n");
            return _crossCache.getInstance(key);
        }
        dump("org.iswas.utils.InstanceCache.create[new Instance for key=" + key + "]\n");
        var instance = new Instance();
        _crossCache.setInstance(key, instance);
        return instance;
    }
    /**
     * Constructor
